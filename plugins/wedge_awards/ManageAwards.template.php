...
function template_modify()
{
	...
	echo '
				<form action="', $scripturl, '?action=admin;area=awards;sa=modify" method="post" name="award" id="award" accept-charset="', $context['character_set'], '" enctype="multipart/form-data">
					<div class="cat_bar">
						<h3 class="catbg">
							', (isset($_GET['saved']) ? $txt['awards_saved_award'] : ($context['editing'] == true ? $txt['awards_edit_award'] . ' - ' . $context['award']['name'] : $txt['awards_add_award'])), '
						</h3>
					</div>';
	...
	echo '
					<div class="windowbg2">
						...
						<div class="content">
							<dl class="settings">
								<dt>
									<label for="name">', $txt['awards_badge_name'], '</label>:
								</dt>
									<dd>
										<input type="text" name="name" id="name" value="', $context['award']['name'], '" size="30" />
									</dd>
								<dt>
...
function template_assign(){
	...
	foreach ($context['awards'] as $key => $award)
		echo '
									<option value="', $key, '" id="', $award['image'], '" ', isset($_REQUEST['id']) &&  $_REQUEST['id'] == $key ? 'selected="selected"' : '', '>', $award['name'], '</option>';

	echo '
								</select>
							</dt>
								<dd>
									<img id="awards" src="', isset($_REQUEST['award']) ? dirname($scripturl) . '/' . $modSettings['awards_dir'] . '/' . $context['awards'][$_REQUEST['award']]['image'] : '', '" align="middle" alt="" />
								</dd>
							<dt>
...
function template_edit_category(){
	...
	echo '
				<form action="', $scripturl, '?action=admin;area=awards;sa=editcategory" method="post" name="category" id="category" accept-charset="UTF-8">
					<we:cat>
					...
					<div class="roundframe">
						<dl class="settings">
							<dt>
								<label for="name">', $txt['awards_category_name'], '</label>:
							</dt>
							<dd>
								<input type="text" name="name" id="name" value="', $context['category']['name'], '" size="30" />
							</dd>
						</dl>
