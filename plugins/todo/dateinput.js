	...
	function Dateinput(input, uconf)
	{
		var
			...
			show = function(e) {
				...
				root.animate(is_opera ? { opacity: 'show' } : { opacity: 'show', height: 'show' }, 100);
				onShow();
      ...
      */
			...
			hide = function()
			{
				if (opened)
				{
					...
					// do the hide
					root.animate(is_opera ? { opacity: 'hide' } : { opacity: 'hide', height: 'hide' }, 100);
					opened = false;
				}
		...
		input.addClass('dateinput');
			...
			root = $('<div><div><div/></div><table><tbody/><tr/></table></div>')
				.addClass('cal').insertAfter(input);
			...
			var monthSelector = $('<select/>').attr('id', 'm').change(function() {
				setValue(yearSelector.val(), $(this).val());
				}),
				yearSelector = $('<select/>').attr('id', 'y').change(function() {
					setValue($(this).val(), monthSelector.val());
				});
			title.html(monthSelector.add(yearSelector));

			for (var d = 0; d < 7; d++)
				days.append($('<th/>').addClass('right').text(daysShort[(d + (conf.firstDay || 0)) % 7]));
